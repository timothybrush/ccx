...
| 策略快照 | `policyFingerprint`、`mode`、`policyRevision`（如已有配置版本） | 指纹由模式、权重、启用约束和策略版本的规范化、无密钥表示计算；用于解释当时决策，不保存整个配置。 |
...
`Match` 的语义固定为：只有 shadow 建议与可比较的实际渠道都存在时才计算；没有比较对象必须表示为“未比较”，而不是 `false`。列表统计应据此排除未比较记录，避免把缺失数据误算为 mismatch。

### 3.2 脱敏与可回放边界

可回放是“重新解释已记录的决策”，不是重新执行请求。详情必须足以回答候选为何被筛除、Autopilot 推荐什么、Scheduler 为何选择、实际做过哪些尝试及其终态，但不得带入用户载荷。

- 允许：`ChannelUID`、模型名、任务类别、能力/过滤原因代码、脱敏 `metricsKey`、状态码、时延桶或精确时延、策略指纹、短 prompt hash。
- 禁止：API Key、`Authorization`、Cookie、完整 URL query、prompt/消息全文、工具参数、响应正文、multipart 名称与内容、原始上游错误 body。
- `PromptHash` 仅可用于内部关联和测试断言；从列表、详情、导出和浏览器响应全部删除。持久化时可保留短期散列的必要性必须单独记录理由，否则 v2 不写入它。
- endpoint 标识应使用渠道 UID 加已脱敏的稳定端点标签；原始 `BaseURL` 只能继续留在受现有权限保护的 `ChannelLog`，不能复制到 Trace API。
...
| 接口 | 返回内容 | 约束 |
|---|---|---|
| 列表 | `TraceSummary`：UID、时间、版本、mode、请求类别、比较状态、推荐/实际渠道、终态与轻量异常标记。 | 按 `(createdAt, traceUid)` 游标分页，默认和最大页大小固定；不含候选、策略或 endpoint 尝试。 |
| 详情 | `TraceDetail`：v2 的安全决策、规范化 Scheduler 摘要、endpoint 尝试摘要及终态。 | 仅按 UID 查询；记录不存在、已过期或未被持久化时返回明确的 `404`，不得从内存泄露未脱敏对象。 |
| 统计 | 以持久化窗口聚合为准，补充 `compared`、`uncompared`、mismatch、失败和 fail-open 数。 | 不以详细 trace 抽样结果推导发布门槛；无偏窗口表仍是 readiness 的事实源。 |
...
前端将 `AutopilotTraceTable` 改为摘要行：点击后由 `AutopilotView` 请求详情并在单一只读详情抽屉中展示“决策 -> Scheduler 裁决 -> upstream 尝试 -> 终态”时间线。请求中显示固定尺寸 loading 状态，`404` 显示记录已过期/未采样，网络错误可重试；页面绝不缓存或渲染未脱敏的后端对象。现有本地 mismatch 开关改为服务端过滤，以确保分页和统计口径一致。

### 3.4 回填顺序

1. 在请求画像和 Autopilot 计划产生时创建 v2 trace，并把 UID 写入 request context；shadow 和 dry-run 都走这一步。
...
新增 `(request_correlation_id, created_at)`、`(mode, created_at)` 和 `(persistence_class, created_at)` 索引；不为 JSON 内部字段建索引，也不把完整候选数组拆成独立关系表。列表所需的字段继续读顶层列，详情只在按 UID 查询时解析 `details_json`。
...
初始 trace 可以因正常抽样而未入库；当随后出现上述终态或关联时，`RecordOutcome`/回填路径必须把内存快照提升为持久化记录，而非只执行 `UPDATE`。同一 `traceUid` 的多次回填用 UPSERT 合并，且不得把已有的完整 `details_json` 覆盖为较早的局部快照。
...
扩展现有 `AutopilotRoutingConfig`，而不是新增另一个发布数据库：增加受校验的 `rolloutPercent`，将 `active` 纳入 `AutopilotMode`、配置归一化、管理 API 和前端类型。`RoutingMode` 表示目标状态，单次请求记录实际生效状态；因此 v2 trace 的策略快照补充 `targetMode`、`effectiveMode`、`rolloutPercent` 和无敏感分桶标识。
...
| `active` | 与 `auto` 的路由语义相同，但代表已完成 100% 发布并接受持续 SLO 守护。 | 固定 100%，不允许保存其他比例。 |

管理员只能逐级晋升，不能从 `off` 直接跳到 `auto/active`。允许随时降级到较低状态，`killSwitch` 和 `AUTOPILOT_KILL_SWITCH` 始终直接生效为 `off`。`rolloutPercent` 仅在 `assist` 与 `auto` 有效，校验范围为 1-100；`shadow/off` 强制为 0，`active` 强制为 100。
...
3. 对剩余请求，优先使用服务器端 session ID，否则使用内部 request ID，与 `policyFingerprint` 一起做稳定哈希分桶；不得使用 prompt、API Key 或客户端任意 header。
4. 命中分桶才以 `assist` 或 `auto` 生效，未命中请求统一执行 `shadow` 并记录比较数据。
...
- `shadow -> assist`：至少观察 24 小时、累计 500 个可比较样本；成功率、fallback、fail-open、p95 延迟均达到现有安全阈值，且没有未解决的关键 mismatch。
...
- `auto -> active`：要求 auto 已在当前策略指纹下完成一整段 24 小时观察、至少 500 个请求且最近没有 rollback；聚合查询参数化为目标模式，避免用 shadow 样本冒充 active 证据。

每个状态的灰度比例只按 `1% -> 5% -> 25% -> 50% -> 100%` 递增。两次递增之间至少完成 3 个 15 分钟窗口，每个窗口至少 20 个终态请求且没有 regression；低流量时保持当前比例，不用时间到期代替样本门槛。比例提升是人工确认动作，系统只负责给出 readiness 和建议，不自动扩大生产流量。
...
复用 `EvaluateAutoRegression` 的 15 分钟窗口和 7 天 shadow/assist 基线：连续 3 个已完成窗口、每窗至少 20 个请求且同时满足 regression 条件时触发降级。普通成功率/fallback/p95 回归将 `auto/active -> assist`；fail-open 突增、硬约束异常或策略不可解释时直接 `auto/active -> shadow`。写入 `AutoSafetyEvent` 后禁止自动恢复，必须重新通过晋升门槛。
...
- 表驱动测试 `SanitizeMetricsKey`、`SanitizeForPersistence/Response`，逐项扫描 JSON 和错误日志中禁止的 key、Authorization、prompt、multipart 内容。
- 测试 v1 -> v2 读模型适配、v2 DTO 白名单、policy fingerprint 在 map 排序和字段顺序变化后仍稳定；未知版本只读且不被覆盖。
- 测试 stable bucket 在同一 request/session/policy 下不漂移，比例边界覆盖 0、1、99、100；受保护选择永远返回 bypass。
- 测试状态迁移只允许相邻晋升、降级总是允许、active 只能 100%，readiness 不足返回阻断原因；三窗口回归按严重度选择 assist 或 shadow。
- 测试 `Match` 的 true/false/uncompared 三态、fallback 分类、重复 `RecordOutcome` 幂等和“抽样后异常提升”判定。
...
- TraceStore `Record`、详情序列化、`ChannelLog` 关联和窗口写入逐个注入错误，确认代理返回与没有观测组件时一致。
- 模拟连续 upstream 失败、fallback、取消和流式中断，确认终态只写一次，且异常样本被提升持久化。
...
- 从模拟 v5 库迁移到 v6，验证版本、列、索引和旧 trace 均保留；重复迁移为 no-op，未来版本仍触发既有 fail-open 初始化路径。
- 普通成功抽样、mismatch/失败/fail-open/manual/advisor/dry-run 必落盘，以及先未采样后在终态提升为异常记录。
- `details_json` 损坏时只跳过该条或返回受控详情错误，不能让列表、其他 trace、Scheduler 或服务启动失败。
...
为 `GET /api/autopilot/traces`、`GET /api/autopilot/traces/:traceUid`、`/stats` 和发布配置路由编写 `httptest`：分页游标、mismatch/状态过滤、空结果、过期 `404`、数据库不可用、权限继承、readiness `409`、非法状态/比例均应有稳定响应。
...
### Task 1：定义 Trace v2 DTO 与脱敏边界

**文件：** `backend-go/internal/autopilot/routing_trace.go`、`backend-go/internal/autopilot/routing_trace_test.go`，必要时新增同包的 `trace_contract.go`。

- [ ] 把 `RoutingDecisionTrace` 的运行时字段映射到显式 `TraceDetailV2`、`TraceSummary`、`SchedulerDecisionSummary`、`EndpointAttemptSummary`；补 `schemaVersion`、`requestCorrelationId`、`policyFingerprint`、target/effective mode 和 rollout 元数据。
- [ ] 保留 `RoutingDecisionTrace` 作为唯一事实源；禁止创建 `RoutingDecisionRecord` 或将 `SelectionTrace`/`ChannelLog` 深拷贝成第二份事实模型。
- [ ] 用 `crypto/rand` 生成碰撞安全 `traceUid`；策略指纹使用排序后的无密钥配置摘要和 SHA-256，不能把 prompt、key、header 或完整 URL 放入输入。
- [ ] 分离 `SanitizeForPersistence` 与 `SanitizeForResponse`，使用白名单 DTO；v1 读取适配器对缺失字段返回“不可用”而非伪造默认值。
- [ ] 补齐 L1 表驱动测试：字段白名单、禁止值扫描、稳定指纹、三态 match、终态幂等、未知 schema 和空输入。

**完成条件：** 任何运行时 trace 都能生成 v2 安全 DTO；序列化/脱敏失败有最小安全结果，调用方无需捕获观测异常即可继续。

### Task 2：实现 SQLite v6 迁移、持久化与清理

**文件：** `backend-go/internal/autopilot/schema_migration.go`、`routing_trace.go`、`routing_readiness.go`，以及对应的 `*_test.go`。

- [ ] 新增 v5->v6 幂等迁移、v2 建表列和索引；在同一事务内完成补列/索引，最后才更新 `PRAGMA user_version`，未来版本保持现有 fail-open。
- [ ] 将安全 DTO 写入 `details_json`，顶层列只保留列表/筛选索引；实现按 UID 详情、游标列表和 v1/v2 读适配，避免启动时全表回写。
- [ ] 把正常成功保留为 1/10 抽样；将 mismatch、失败、耗尽、fallback/fail-open、manual/advisor 和显式 dry-run 统一标记为必落盘类别。
- [ ] 在终态回填时把“先未采样、后变异常”的内存记录提升为 UPSERT；详情合并必须防止旧局部快照覆盖完整新快照。
- [ ] 实现 7 天详细 trace、30 天窗口/event 清理，启动一次并按 24 小时门限批量执行；清理/DB 错误只告警并 fail-open。
- [ ] 补 SQLite 临时文件测试：迁移、重启还原、坏 JSON、过期边界、重复清理、关闭/锁库和高版本 schema。
...
- [ ] 在公共多渠道 failover 外壳最早处生成一次 `requestCorrelationId`，用私有 Gin context key 传递；不得复用客户端 header，也不得替换每次 endpoint 尝试已有的 `ChannelLog.RequestID`。
- [ ] 让 real、shadow、assist、auto、active 和显式 dry-run 共用同一“构建决策 + 创建 trace”路径；`BuildPlan` 也必须写入 dry-run trace，不能只返回临时 plan。
- [ ] 将 Scheduler 的 `SelectionTrace` 规范化后附到对应 trace；无论选择成功还是得到 `SelectionTraceError`，均保留已知的硬约束阶段与原因。
- [ ] 为 `ChannelLog` 增加 `RequestCorrelationID` 和 `AutopilotTraceUID`，通过新增 `ChannelLogOption` 从公共尝试路径写入；补写被环形缓冲淘汰的终态日志也必须继承两个字段。
- [ ] 在 endpoint 尝试开始/结束时向 TraceStore 追加安全尝试摘要；由外层请求完成路径一次性回填终态与窗口，避免每次 key retry 重复终结同一 trace。
- [ ] 对 trace 生成、回填、摘要持久化的 panic/错误做局部保护，返回原 Scheduler/Failover 结果；不得在 messages/chat/responses/gemini 各自复制一套实现。
- [ ] 补集成测试覆盖多 key、多渠道 failover、stream cancel、SelectionTraceError、无 Autopilot、记录失败和一次逻辑请求关联多条 ChannelLog。
...
- [ ] 将 `active`、`rolloutPercent` 加入 Autopilot 配置 schema、归一化、持久化 setter 和安全响应；保留旧配置默认 shadow，非法状态/比例回退到安全值。
- [ ] 新建小型 `ReleaseController`，集中处理相邻状态迁移、stable bucket、目标/实际 mode、readiness、人工比例递增和运行态安全覆盖；SmartRouter 只消费其结果，不自行判断发布状态。
- [ ] 用 session ID 或内部 request correlation ID 做稳定哈希；先应用保护选择与硬约束，再决定分桶，未命中分桶强制 shadow。
- [ ] 参数化现有 readiness/回归聚合，复用 24 小时、500 样本、成功率、fallback、fail-open 和 p95 基线阈值；把比例提升限制为 `1/5/25/50/100` 和三个完整窗口。
- [ ] 在每个完成窗口后低频检查 regression：连续三窗回归降为 assist 或 shadow，先设进程内覆盖再最佳努力保存配置和 `AutoSafetyEvent`，不自动恢复。
- [ ] 补纯函数和 HTTP 测试：非法跳级、比例边界、稳定分桶、所有显式控制优先、readiness 409、三窗降级、配置保存失败时的安全覆盖。

**完成条件：** 不存在绕过状态机的高风险晋升；未放量和受保护请求保持 shadow/原调度；SLO 回归能在当前进程立即停止 auto/active 影响。

### Task 5：提供只读 Trace API 与驾驶舱详情
...
- [ ] 将列表响应改为 `TraceSummary`，实现 `(createdAt, traceUid)` 游标、服务端过滤和固定页大小；统计从窗口聚合读取，不从抽样详情外推。
- [ ] 新增按 UID 的只读详情 handler，统一从安全读 DTO 获取数据；过期/未采样返回 `404`，数据库暂不可用返回受控错误，绝不回退序列化内存原始 trace。
- [ ] 以与现有 Autopilot 管理 API 相同的路由组和鉴权注册接口；不增加删除、下载、重放、写策略或代理侧 endpoint。
- [ ] 扩展 API types 和客户端方法；移除列表中完整候选数组，避免初始加载过大和页面持有详情数据。
...
- [ ] 用 `httptest` 断言 API 脱敏、分页、404、授权继承和统计口径；以前端类型检查/构建验证详情交互契约。
...
- [ ] 为 messages/chat/responses/gemini 的本地 fake upstream 编写非流式与 SSE golden 回归，比较 trace 启用前后的响应帧、状态码、header、取消与 failover 语义。
...
- [ ] 更新 `channel-autopilot.md` 中 trace schema、保留期、rollout/active 定义、回退语义与测试入口，删除与实现不一致的“所有 dry-run 已自动记录”等陈述。
...
- 发布前确认 TraceStore 初始化、v6 migration、readiness 窗口和管理 API 均健康；任一项不可用时保持 off/shadow，不直接放量。
- shadow 阶段观察 24 小时和至少 500 样本，检查 mismatch、uncompared、失败、fail-open、p95 和每种协议/TaskClass 的分布。
- 每次比例提升记录策略指纹、操作者和前后状态；若低流量无法形成三个完整窗口，保持原比例而非强行晋升。
- auto/active 期间验证三窗口回归能把实际生效模式降到 assist/shadow；回滚后不得自动恢复，需人工重新通过 readiness。
...
| 风险 | 影响 | 缓解与阻断条件 |
|---|---|---|
| 关联 ID 在 retry/failover 中丢失 | 详情无法还原完整尝试链。 | 外层生成逻辑请求 ID，ChannelLog 保留 attempt ID；L2 强制测试一请求多尝试。 |
| 新字段意外包含敏感内容 | SQLite/API/浏览器泄露密钥或用户数据。 | 显式持久化/响应 DTO、双重脱敏、sentinel 扫描；任一泄露测试失败即阻断发布。 |
| 抽样在终态前完成 | 失败或 fail-open 样本重启后消失。 | 终态分类触发升级落盘；SQLite 重启测试必须验证。 |
| SQLite migration 或清理锁库 | Autopilot 启动失败或请求延迟。 | migration 失败禁用 Autopilot；清理小批量、观测错误 fail-open；不在请求关键路径同步清理。 |
| rollout 分桶不稳定 | 同一 session 在不同请求被反复切换。 | 只用 session/request correlation + policy fingerprint 的稳定 hash；改策略时指纹变化并在 trace 显示。 |
| 显式控制被自动路由覆盖 | 用户指定渠道、协议或模型行为改变。 | 保护层先于分桶/排序；`X-Channel`、manual、promotion、session pin 和硬约束有独立不变量测试。 |
| active 或配置迁移被旧代码忽略 | 管理面显示已发布但真实模式不一致。 | 配置归一化、SmartRouter、readiness、API、前端类型同时更新；未知值安全回 shadow。 |
| SLO 回归检测滞后或误判 | 高风险流量持续，或频繁抖动。 | 三个完整 15 分钟窗口、每窗最小样本、7 天基线；严重硬约束/观测故障直接 shadow，回滚后人工恢复。 |
...
**最终验收：** 三类方法（可观测性契约、shadow/灰度发布、分层测试）均有对应实现任务、测试断言、运行命令和回滚条件；OpenSquilla 只作为方法论来源，不改变 CCX 的代理协议和调度所有权。
